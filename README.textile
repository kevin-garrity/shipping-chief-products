...
The following files are copied into the theme autmatically.
...
Wherever you find @{{ cart.item_count }}@ in the layout/theme.liquid file and layout/cart.liquid replace it with:

@{% include 'webify-shipping-items-hidden-price' %}@
...
Add the following to templates/cart.liquid, after the cart form near the bottom of the file. Look for proceed to checkout button and add the following line

@{% include 'webify-request-shipping-form' %}@


Add item class to each product tr in cart.liquid
...
For theme that does not use table for cart, add id="cart-subtotal" to the line that contains subtotal. If total is in a table row, create an empty div with the id cart-subtotal. Check if shipping charge is displayed before subtotal in the shop.
...
tag td with class name price, quantity, total, item for calculator to work
(TODO Ziggy thinks we should use 'sub-total' instead of 'total')

edit selects at var selectors in webify.ajaxify-shop.js to find cart total
